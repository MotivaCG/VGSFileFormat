/*
 * TweetNaCl 20140427, vendored unchanged from https://tweetnacl.cr.yp.to/
 *
 * Written by Daniel J. Bernstein, Bernard van Gastel, Wesley Janssen, Tanja Lange,
 * Peter Schwabe and Sjaak Smetsers, and placed by them in the public domain.
 *
 * Public domain means there is nothing to comply with: no notice to reproduce, no
 * attribution to carry, no terms to pass on. The provenance is recorded here so that
 * anyone auditing a binary this ends up inside can trace where the cryptography came
 * from, which is a different question from whether they owe anybody anything.
 *
 * It is the reference implementation and is not ours to tidy: it is left exactly as
 * published, and the build turns its warnings off rather than edit it.
 */
#include "tweetnacl.h"
